class CloneBatchesController < InheritedResources::Base
  ...
  def index
    ...
    #variable global utilisé par la méthode 'listing' pour eviter l'initialisation de la recherche à la fermeture de la fenêtre modale (edit-from-inventory)
      records = @q.result.includes(:clone, :target, :type, :strand, :genes, :promoters, :origin, :plasmid_batches).where.not(:nb => nil).where.not(:id => hidden_plasmids_ids)
    ...
    #Config de l'affichage des résultats.
      ...
      @pagy, @clone_batches = pagy(records.order(nb: :desc), items: 30)

    respond_to do |format|
      format.html
      format.js
      format.csv {send_data records.to_csv, :filename => "plasmids-#{Date.today.strftime('%m/%d/%Y')}.csv"}
    end
